...
void processInput(GLFWwindow* window) // Fonction pour les input (quand on presse une touche)
{
	...
	if (glfwGetKey(window, GLFW_KEY_C) == GLFW_PRESS) // GLFW_PRESS permet de vérifier si la touche en question est pressé
		std::cout << "Vous avez appuye sur la touche C !" << std::endl; // Ptit message 
}
...
int main() 
{
	...
	while (!glfwWindowShouldClose(fenetre))//vérifie au début de chaque itération de boucle si GLFW a reçu l'ordre de fermer
	{
		...
		// Rendering commands :
		glClearColor(0.2f, 0.3f, 0.3f, 1.0f); // On choisis une couleur : red, green, blue, alpha;
		glClear(GL_COLOR_BUFFER_BIT);
		/*
		Nous pouvons effacer le tampon de couleur de l'écran en utilisant glClear où nous passons des bits de tampon pour spécifier quel tampon nous aimerions effacer.
		Les bits possibles que nous pouvons définir sont GL_COLOR_BUFFER_BIT, GL_DEPTH_BUFFER_BIT et GL_STENCIL_BUFFER_BIT. 
		À l'heure actuelle, nous ne nous soucions que des valeurs de couleur, nous n'effaçons donc que le tampon de couleur.

		La fonction glClearColor est une fonction de définition d'état 
		glClear est une fonction d'utilisation d'état en ce sens qu'elle utilise l'état actuel pour récupérer la couleur d'effacement.
		*/
